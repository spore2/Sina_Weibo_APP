...
public class StatusesAPI extends AbsOpenAPI {
    ...
    private static final int WRITE_API_SHARETEXT       = 6;
    private static final int WRITE_API_SHAREIMAGE      = 7;
    private static final int WRITE_API_DELETE          = 8;
    ...
    static {
        ...
        sAPIList.put(WRITE_API_SHARETEXT,       API_BASE_URL + "/share.json");
        sAPIList.put(WRITE_API_SHAREIMAGE,      API_BASE_URL + "/share.json");
        sAPIList.put(WRITE_API_DELETE,          API_BASE_URL + "/share.json");
    }
    ...
    /*
     *发布一条微博（自定义的备用接口，之前的update接口被官方给ban了   orz...）
     */
    public void shareText(String content, String lat, String lon, RequestListener listener){
        WeiboParameters params = buildUpdateParams(content, lat, lon);
        requestAsync(sAPIList.get(WRITE_API_SHARETEXT), params, HTTPMETHOD_POST, listener);
    }
    ...
    /*
     *上传图片并发布一条微博，同样是自定义的替换接口，之前官方提供的接口（见上）又被ban了
     */
    public void shareImage(String content, Bitmap bitmap, String lat, String lon, RequestListener listener) {
        WeiboParameters params = buildUpdateParams(content, lat, lon);
        params.put("pic", bitmap);
        requestAsync(sAPIList.get(WRITE_API_SHAREIMAGE), params, HTTPMETHOD_POST, listener);
    }
